...
const Planner = ({ navigation }) => {
  ...
  const handleTimeChange = useCallback((index, type) => {
    return (value, timeType) => {
      setTimeSlots(prevSlots => {
        ...
        if (newEnd <= newStart) {
          translateText("Invalid Time", language).then(title => {
            translateText("End time must be after start time", language).then(msg => {
              Alert.alert(title, msg);
            });
          });
        } else if ((newEnd - newStart) < 60) {
          translateText("Invalid Duration", language).then(title => {
            translateText("Minimum slot duration is 1 hour", language).then(msg => {
              Alert.alert(title, msg);
            });
          });
        } else if ((newEnd - newStart) > 180) {
          translateText("Invalid Duration", language).then(title => {
            translateText("Maximum slot duration is 3 hours", language).then(msg => {
              Alert.alert(title, msg);
            });
          });
        }
  ...
  const addTimeSlot = useCallback(() => {
    if (timeSlots.length >= 5) {
      translateText("Limit Reached", language).then(title => {
        translateText("Maximum 5 time slots allowed", language).then(msg => {
          Alert.alert(title, msg);
        });
      });
      return;
    ...
    if (!validateTimeSlot(newSlot)) {
      translateText("Invalid Slot", language).then(title => {
        translateText("This slot overlaps with existing slots", language).then(msg => {
          Alert.alert(title, msg);
        });
      });
      return;
  ...
  }, [timeSlots, validateTimeSlot, language]);

  const deleteTimeSlot = useCallback((index) => {
    if (timeSlots.length <= 1) {
      translateText("Cannot Delete", language).then(title => {
        translateText("At least one time slot is required", language).then(msg => {
          Alert.alert(title, msg);
        });
      });
      return;
    ...
    return (
      <ScrollView className="flex-1 w-screen" nestedScrollEnabled={true}>
        <View className="p-5 bg-pink-50 flex-1">
          <TranslatedText text="Craft Your Ideal Study Plan" className="text-2xl mb-4 text-[#BD835D] font-bold" />
          <TranslatedText text="Choose your deadline:" className="text-xl font-bold mb-2 ml-2 text-[#bd835dc1]" />

          <View className="flex-row gap-2 mb-6 ml-2 mt-2">
            <View className="flex-1">
              <TranslatedText text="From" className="mb-2 text-[#BD835D] font-bold" />
              <DateTimeSelector
                mode="date"
            ...
            <View className="flex-1">
              <TranslatedText text="To" className="mb-2 text-[#BD835D] font-bold" />
              <DateTimeSelector
                mode="date"
          ...
          <View className="flex-row justify-between items-center mb-2">
            <TranslatedText text="Time Slots" className="text-xl font-bold ml-2 text-[#BD835D]" />
            <TouchableOpacity
              className="bg-pink-400 py-2 px-4 rounded mt-4 mx-2 items-center"
              onPress={addTimeSlot}
            >
              <TranslatedText text="+ Add Slot" className="text-white font-bold" />
            </TouchableOpacity>
          </View>
          ...
          {timeSlots.map((time, index) => (
            <View key={`timeslot-${index}`} className="mb-4 ml-2">
              <View className="flex-row justify-between items-center">
                <TranslatedText text={`Slot ${index + 1}:`} className="font-medium" />
              ...
              <View className="flex-row gap-2 mt-2">
                <View className="flex-1">
                  <TranslatedText text="From:" className="text-gray-600 mb-1" />
                  <CustomTimePicker
                    key={`from-${index}`}
                    Hour={time.from.hour}
                ...
                <View className="flex-1">
                  <TranslatedText text="To:" className="text-gray-600 mb-1" />
                  <CustomTimePicker
                    key={`to-${index}`}
                    Hour={time.to.hour}
  ...
  const SubjectsSlide = memo(() => {
    ...
    const handleSave = () => {
      if (!newSubject.trim()) {
        translateText("Required", language).then(title => {
          translateText("Subject name is required", language).then(msg => {
            Alert.alert(title, msg);
          });
        });
        return;
      ...
      if (topicsArray.length === 0) {
        translateText("Required", language).then(title => {
          translateText("At least one topic is required", language).then(msg => {
            Alert.alert(title, msg);
          });
        });
        return;
    ...
    const handleRemoveTopic = (subjectIndex, topicIndex) => {
      if (subjects[subjectIndex].topics.length <= 1) {
        translateText("Cannot Remove", language).then(title => {
          translateText("At least one topic is required", language).then(msg => {
            Alert.alert(title, msg);
          });
        });
    ...
    return (
      <View className="flex-1 bg-pink-50 p-5 w-screen">
        <View className="flex-row justify-between items-center mb-4">
          <TranslatedText text="Subjects & Topics" className="text-xl font-bold text-[#BD835D]" />
          <TouchableOpacity
            className="bg-blue-500 py-2 px-4 rounded"
            onPress={handleAddSubject}
          >
            <TranslatedText text="+ Add Subject" className="text-white font-bold" />
          </TouchableOpacity>
        </View>

        {subjects.length === 0 ? (
          <View className="flex-1 justify-center items-center mt-10">
            <TranslatedText text="No subjects added yet" className="text-gray-500" />
          </View>
        ) : (
          <ScrollView className="flex-1">
            {subjects.map((subject, subjectIndex) => (
              <View key={`subject-${subjectIndex}`} className="bg-pink-100 rounded-lg p-4 mb-3">
                <View className="flex-row justify-between items-center mb-2">
                  <TranslatedText text={subject.name} className="text-lg font-semibold" />
                ...
                <View className="flex-row flex-wrap gap-2">
                  {subject.topics.map((topic, topicIndex) => (
                    <View key={`topic-${topicIndex}`} className="flex-row items-center bg-gray-200 py-1 px-3 rounded-full">
                      <TranslatedText text={topic} className="mr-1" />
        ...
        >
          <View className="bg-white rounded-t-xl p-6" style={{ borderTopLeftRadius: 20, borderTopRightRadius: 20 }}>
            ...
            <View className="flex-row justify-between mt-3">
              ...
              >
                <TranslatedText text="Cancel" className="text-white font-bold" />
              </TouchableOpacity>
  ...
  const DifficultySlide = memo(() => {
    ...
    return (
      <View className="flex-1 bg-pink-50 p-5 w-screen">
        <TranslatedText text="Set Subject & Topic Proficiency" className="text-xl font-bold mb-4" />

        <ScrollView className="flex-1">
          {/* Subjects Section */}
          <View className="mb-8">
            <TranslatedText text="Subjects Proficiency" className="text-lg font-bold mb-3" />
            {subjectsWithDifficulty.length === 0 ? (
              <TranslatedText text="No subjects added" className="text-gray-500 text-center" />
            ) : (
              subjectsWithDifficulty.map((subject, subjectIndex) => (
                ...
                >
                  <TranslatedText text={subject.name} className="text-lg flex-1" />
          ...
          {/* Topics Section */}
          <View>
            <TranslatedText text="Topics Proficiency" className="text-lg font-bold mb-3" />
            {subjectsWithDifficulty.length === 0 ? (
              <TranslatedText text="No subjects available" className="text-gray-500 text-center" />
            ) : (
              subjectsWithDifficulty.map((subject, subjectIndex) => (
                <View key={`topic-group-${subjectIndex}`} className="mb-6">
                  <TranslatedText text={subject.name} className="font-semibold mb-2" />

                  {subject.topics.length === 0 ? (
                    <TranslatedText text="No topics added" className="text-gray-500 italic" />
                  ) : (
                    subject.topics.map((topic, topicIndex) => (
                      <View
                      ...
                      >
                        <TranslatedText text={topic.name} className="flex-1" />
  ...
  const ReviewSlide = memo(() => {
    return (
      <View className="flex-1 bg-pink-50 p-5 w-screen">
        <TranslatedText text="Review & Confirm" className="text-xl font-bold mb-4" />

        <TranslatedText text={`From: ${selectedDate.toDateString()}`} className="mb-2" />
        <TranslatedText text={`To: ${endDate.toDateString()}`} className="mb-6" />

        <TranslatedText text="Time Slots:" className="font-bold mb-2" />
        ...
        <TranslatedText text="Subjects:" className="font-bold mt-4 mb-2" />
        ...
        <TouchableOpacity
          ...
          onPress={async () => {
            try {
              ...
              console.log('Planner generated:', data);

              // Save to Appwrite
              const user = await getCurrentUser();
              if (user) {
                await savePlanner(user.$id, JSON.stringify(data), subjects.length);
              }

              // Navigate to the Community screen to show the planner
              router.replace('/(tabs)/(community)');
            } catch (error) {
              console.error('Error generating planner:', error);
              Alert.alert('Error', 'Failed to generate planner. Please try again.');
            } finally {
              setIsLoading(false);
            }
          }}
        >
          {isLoading ? (
            ...
            <TranslatedText text="Generate Study Planner" className="text-white font-bold text-lg" />
          )}
        </TouchableOpacity>
      </View>
    );
      ...
      <View className="flex-row justify-end p-3">
        {currentIndex > 0 && (
          ...
          >
            ...
            <TranslatedText text="Previous" className="font-bold text-[#AA7589]" />
          </TouchableOpacity>
        )}
        {currentIndex < slidesArray.length - 1 && (
          <TouchableOpacity
            ...
            onPress={nextSlide}
          >
            <TranslatedText text="Next" className="font-bold text-[#AA7589]" />
